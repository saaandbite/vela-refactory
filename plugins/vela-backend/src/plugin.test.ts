...
import { velaBackendPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [velaBackendPlugin],
    });

    await request(server).get('/api/vela-backend/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/vela-backend/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/vela-backend/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/vela-backend/todos/${createdTodoItem.id}`)
  ...
  it('should forward errors from the TodoListService', async () => {
    ...
    const listRes = await request(server).get('/api/vela-backend/todos');
    expect(listRes.status).toBe(401);
    expect(listRes.body).toMatchObject({
      error: { name: 'AuthenticationError' },
    });

    const getRes = await request(server).get('/api/vela-backend/todos/123');
